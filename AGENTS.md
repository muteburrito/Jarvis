...
- `internal/workbench/project_activity.go` owns per-project command policy, command history, and edit history storage. Approved command runs must be recorded there before they are exposed to future model tool loops.
...
- Start with project-scoped state: vector store, chat history, workspace map, command policy, and edit history per project. Project vector stores, project-tagged chats, and project command history are in place; edit history and patch state still need project isolation.
