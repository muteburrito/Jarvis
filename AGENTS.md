...
- `internal/websearch/` contains the DuckDuckGo scraper (`duckduckgo.go`) and the research orchestrator (`research.go`). Research mode emits structured progress events while it searches, fetches, and indexes pages. Query generation includes browser locale, timezone, local date, and local time. Normal chat may use the same pipeline quietly only for clearly time-sensitive live-context questions.
- `internal/workbench/` contains the Codex-style memory layer: chat sessions, watched folders, workspace maps, symbol indexes, local git diff summaries, task messages, traces, and edit history persisted as JSON under the data directory. Workspace maps include regular documents, PDFs, images, spreadsheets, presentations, data files, source files, code symbols, image dimensions, package summaries, test counts, and top-level folder groups.
...
- `internal/server/agent_tools.go` owns bounded read-only model tool planning for chat. It lets the chat model request active-project `search_files`, `summarize_file`, and `read_file` calls, validates the JSON plan, records traces, and falls back to deterministic file summaries when planning fails.
...
- Folder ingest builds a lightweight workspace map for regular files and code. It classifies documents, PDFs, spreadsheets, presentations, images, data, config, text, and code files. It extracts symbols/imports only for supported text/code files and records image dimensions, package summaries, test counts, extensions, directories, and top-level folder groups. Keep scanners fast and dependency-free unless the roadmap explicitly moves to tree-sitter.
...
- When documents are indexed, generic questions should still answer as general chat. Use indexed context only when the user explicitly focuses files, mentions a known filename, asks about documents/files/attachments, or clearly refers to the active project/repo/codebase.
- Jarvis picks the chat model automatically when `CHAT_MODEL` is unset: use `gemma4:e2b` for very low-end machines and `gemma4:e4b` for everyone else. Do not auto-select `gemma4:26b`; it is opt-in because Jarvis is local and RAG-first, and speed matters for daily use. The composer dropdown may select Gemma 4 E2B, E4B, or 26B for a request. If the selected dropdown model is missing, Jarvis should pull it from Ollama on demand before answering.
...
- Research mode uses DuckDuckGo HTML scraping (no API key). The LLM generates 2-3 search queries using the user's locale, timezone, date, and local time when available. Results are fetched and indexed, then the RAG chain answers with a research-specific prompt emphasising source citations. Research progress must be sent as structured SSE progress events, not markdown status text mixed into the answer. Gemma 4 thinking mode may enable `<|think|>`, but hidden thinking must never be shown to the user or written back into chat history.
...
- Add read-only tools first: list files, search files, read files, inspect symbols, summarize files, and fetch context. Active-project file search, read, summarize endpoints, and structured read-only chat tool planning are now available.
