...
- `internal/websearch/` contains the DuckDuckGo scraper (`duckduckgo.go`) and the research orchestrator (`research.go`). Research mode emits structured progress events while it searches, fetches, and indexes pages. Normal chat may use the same pipeline quietly for live context without emitting query progress.
